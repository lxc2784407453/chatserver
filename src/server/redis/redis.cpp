...
{
    // SUBSCRIBE 命令会让当前connect阻塞等待通道里面发生消息，这里只做订阅，不接收消息
    // 通道消息的接收由 observer_channel_message 的独立线程执行
    // 这里只负责发送命令，不阻塞 redis server 响应消息，否则会让 notifyMsg 线程占用
    ...
    // redisBufferWrite 可以循环发送缓冲区，直到数据发送完毕
    int done = 0;
    while (!done)
    {
...
void Redis::observer_channel_message()
{
    ...
    while (REDIS_OK == redisGetReply(this->_subscribe_context,
                                     (void **)&reply))
    {
        // 订阅收到的消息是一个带三元素的数组
        if (reply != nullptr &&
